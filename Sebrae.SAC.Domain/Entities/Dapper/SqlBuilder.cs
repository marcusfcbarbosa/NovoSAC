...
namespace Dapper
{
    public class SqlBuilder
    {
        ...
        // HACK: By Charles, adding support for SET list for an UPDATE
        public SqlBuilder Set(string sql, dynamic parameters = null)
        {
            return Set(sql, sql, parameters);
        }

        public SqlBuilder Set(string sql, string sqlForValue, dynamic parameters = null)
        {
            AddClause("set", String.Format("{0} = @{1}", sql, sqlForValue), parameters, " , ", prefix: "SET ", postfix: "\n");
            return this;
        }
        // ENDHACK
