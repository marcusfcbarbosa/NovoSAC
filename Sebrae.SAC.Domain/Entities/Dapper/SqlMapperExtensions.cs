...
namespace Dapper.Contrib.Extensions
{
    ...
    public static class SqlMapperExtensions
    {
        ...
        /// <param name="keyInsert">Inserido por Charles: indica se a inclusão de valor para campo de chave será permitida. Por default, o campo anotado como [Key] nunca será enviado, assumindo-se que o mesmo é um identity.</param>
        /// <returns>Identity of inserted entity or, if keyInsert=true, the same value provided for the key field</returns>
        public static dynamic Insert<T>(this IDbConnection connection, T entityToInsert, Boolean keyInsert = false, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        // HACK: Por Charles, inseri um parâmetro opcional para permitir a inclusão de campos chave (eram ignorados)
        {
            var type = typeof(T);
            ...
            var allPropertiesExceptKey = keyInsert ? allProperties : allProperties.Except(keyProperties);
            ...
            dynamic id = adapter.Insert(connection, transaction, commandTimeout, name, sbColumnList.ToString(), sbParameterList.ToString(), keyProperties, entityToInsert);
            return id;
        }
...
{
    // HACK: Por Charles, permite inserção de campos de chave
    dynamic Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, String tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert);
}

public class SqlServerAdapter : ISqlAdapter
{
    // HACK: Charles Roberto Canato, troquei a forma de recuperar o IDENTITY, praticamente alterei todo o método
    public dynamic Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, String tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        string cmd = String.Format(
            "insert into {0} ({1}) values ({2}); select cast( SCOPE_IDENTITY() as int) AS id;"
            , tableName
            , columnList
            , parameterList
        );

        var response = connection.Query(cmd, entityToInsert, transaction: transaction, commandTimeout: commandTimeout).First();

        Int32 id = 0;
        // Altere a propriedade marcada como [Key] para o valor obtido.
        // O resultado pode ser nulo quando não há inserção que envolva identity
        // Neste caso, não altere o campo [Key] e retorne ZERO.
        if (response.id != null)
        {
            id = response.id;
            if (keyProperties.Any())
